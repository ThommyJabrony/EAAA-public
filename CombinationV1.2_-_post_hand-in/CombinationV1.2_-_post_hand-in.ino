...
bool prevSel = HIGH;
uint8_t switcher = 0;
uint8_t switcherMax = 5;
float effectThresh;


unsigned long serialCheck = 0;
//unsigned long updCheck = 0;
unsigned long readCheck = 0;
  ...
  switch(switcher)
  {
    ...
    case 4: //BITCRUSH DELAY COMBO
      if(effectThresh > 127) 
        ...
        else //STILL BITCRUSH BUT DISABLED
        { 
        if(micros() - readCheck > 50)
          {
            guitarIn = analogRead(inPin);
    ...
    case 5: //FULL WAVE RECT DELAY COMBO
       if(micros() - readCheck > 50)
       {
         guitarIn = analogRead(inPin);
